...
class HistoryAdapter : RecyclerView.Adapter<HistoryAdapter.TugasViewHolder>(){
    ...
    private fun opsilain(view:View,position: Int,holder: TugasViewHolder){
        ...
        popup.inflate(R.menu.popup_history)
        popup.setOnMenuItemClickListener {
            when (it.itemId){
                ...
                R.id.restore->{
                        ...
                        val tugas = Tugas(listtugas[position].id,listtugas[position].judul,listtugas[position].kategori,listtugas[position].status,listtugas[position].deskripsi,"g")
                        tugasViewModel.updateTugas(tugas)

                    Toast.makeText(context,"Berhasil direstore",Toast.LENGTH_SHORT).show()
                    true
                }else->true
            }
        }
